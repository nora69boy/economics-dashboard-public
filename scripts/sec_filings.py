...
USER_AGENT=os.environ.get('SEC_USER_AGENT','EconomicsResearchDashboard/1.1 contact:https://github.com/nora69boy/economics-dashboard-public/issues')
MAX_PER_COMPANY=8
...
def fetch_json(url:str,timeout:float=20.0)->dict:
    req=urllib.request.Request(url,headers={'User-Agent':USER_AGENT,'Accept':'application/json','Accept-Encoding':'identity'})
...
def probe(fetcher=fetch_json,sleep=time.sleep)->dict:
    ...
    for index,ticker in enumerate(COMPANIES):
        ...
        try:companies.append(normalize(ticker,fetcher(BASE.format(cik=cik))))
            ...
            errors.append({'ticker':ticker,'error':error_code(exc)})
        if index<len(COMPANIES)-1:sleep(0.25)
    all_blocked=(not companies and len(errors)==len(COMPANIES) and all(e['error'] in {'HTTP_403','HTTP_429'} for e in errors))
    health='fresh' if len(companies)==len(COMPANIES) else ('source_access_blocked' if all_blocked else 'degraded')
    return {'sec_probe_health':health,'checked_at':datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z'),'publication':'disabled_pending_rights_approval','companies':companies,'errors':errors}
    ...
    if os.environ.get('GITHUB_STEP_SUMMARY'):
        with open(os.environ['GITHUB_STEP_SUMMARY'],'a',encoding='utf-8') as f:
            ...
            for company in result['companies']:
                latest=company['latest'][0];f.write('- '+company['ticker']+': '+latest['form']+' filed '+latest['filing_date']+'\n')
            for error in result['errors']:f.write('- '+error['ticker']+': '+error['error']+'\n')
            if result['sec_probe_health']=='source_access_blocked':f.write('- GitHub-hosted runner source access is blocked/rate-limited; this is not a publication or rights failure.\n')
