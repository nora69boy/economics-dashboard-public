"""Apply the explicit product-owner calendar rights decisions without broadening any other dataset."""
...
def load_approvals(path: Path | None = None):
    ...
    rights.require(isinstance(data, dict) and set(data) == {
        'schema_version', 'approved_at', 'valid_until', 'review_due_at', 'datasets'
    })
    rights.require(data['schema_version'] == 1)
    approved_at = rights.day(data['approved_at'])
    valid_until = rights.day(data['valid_until'])
    review_due_at = rights.day(data['review_due_at'])
    ...
    for item in data['datasets']:
        rights.require(isinstance(item, dict) and set(item) == {
            'id', 'terms_url', 'reference', 'evidence_text', 'attribution_text'
        })
        rights.require(rights.identifier(item['id']) and item['id'] not in seen)
        seen.add(item['id'])
        rights.url(item['terms_url'])
        rights.text(item['reference'])
        rights.text(item['evidence_text'])
        rights.url(item['attribution_text'])
    rights.require(seen == EXPECTED_IDS)
    return data


def effective_registry(base=None, approvals=None):
    base = copy.deepcopy(base or rights.read_json(ROOT / 'data-rights/registry.json'))
    approvals = approvals or load_approvals()
    entries = {entry['id']: entry for entry in base['datasets']}
    rights.require(EXPECTED_IDS <= set(entries))
    for decision in approvals['datasets']:
        ...
        entry.update({
            ...
            'reviewed_at': approvals['approved_at'],
            'evidence': [{
                'reference': decision['reference'],
                'terms_sha256': hashlib.sha256(decision['evidence_text'].encode('utf-8')).hexdigest(),
                'approved_by_role': 'product_owner',
            }],
            'status': 'CONDITIONAL',
            'valid_from': approvals['approved_at'],
            'valid_until': approvals['valid_until'],
            'review_due_at': approvals['review_due_at'],
            'conditions': [{
                'type': 'attribution',
                'text': decision['attribution_text'],
                'targets': ['data/macro-calendar.json', 'index.html'],
            }],
        })
    rights.validate_registry(base)
    return base
