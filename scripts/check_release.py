...
EXTRA_FILES={'scripts/calendar_snapshot.py','scripts/run_legacy_tests.py','scripts/macro_core.py','scripts/macro_fetch.py','scripts/build_release.py','scripts/check_release.py','scripts/browser_release.mjs','scripts/release_assertions.js','templates/macro.js','templates/macro.css','tests/test_release.py','docs/data-rights.md','docs/release-v060.md'}
def validate(site:Path):
 ...
 gate.require(s.count("version:'0.6.0'")==2 and 'macro-chart-' in s,'Actual release program');gate.require((ROOT/'templates/macro.js').read_text() in s,'Macro template mismatch');return payload
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--check-history',action='store_true');ap.add_argument('--build',action='store_true');args=ap.parse_args();out=validate(ROOT/'site')
