...
try{
 const endpoint=await new Promise((ok,bad)=>{let buffer='';const timer=setTimeout(()=>bad(Error('start timeout')),20000);proc.on('error',()=>{clearTimeout(timer);bad(Error('browser unavailable'));});proc.on('exit',()=>{clearTimeout(timer);bad(Error('browser exited'));});proc.stderr.on('data',b=>{buffer=(buffer+b.toString()).slice(-10000);const m=buffer.match(/DevTools listening on (ws:\/\/[^\s]+)/);if(m){clearTimeout(timer);ok(m[1]);}});});
