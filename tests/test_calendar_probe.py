...
NOW_2027_JST=datetime(2026,12,31,15,5,tzinfo=timezone.utc)
...
def combine_bls(title,*years):
 first=bls_html(title,years[0]);rows=b''.join(bls_html(title,y).split(b'<table>',1)[1].split(b'</table>',1)[0] for y in years[1:])
 return first.replace(b'</table>',rows+b'</table>')

BLS_CPI_HTML=bls_html('Consumer Price Index',2026)
BLS_JOBS_HTML=bls_html('Employment Situation',2026)
BLS_CPI_BOTH=combine_bls('Consumer Price Index',2026,2027)
BLS_JOBS_BOTH=combine_bls('Employment Situation',2026,2027)
BLS_CPI_27_28=combine_bls('Consumer Price Index',2027,2028)
BLS_JOBS_27_28=combine_bls('Employment Situation',2027,2028)
...
def bea_year(year,count=12):
 months=list(c.MONTHS);periods=[('December',year-1)]+[(m,year) for m in months[:11]];rows=[]
 for i,(ref,ref_year) in enumerate(periods[:count]):rows.append(f'{months[i]} 15 8:30 AM Personal Income and Outlays, {ref} {ref_year}')
 return ('<html><body>Release Schedule Year '+str(year)+' '+' '.join(rows)+'</body></html>').encode()

BEA_NEXT=bea_year(2027)
BEA_NEXT_PARTIAL=bea_year(2027,11)
BEA_2027=bea_year(2027)
BEA_2028=bea_year(2028)
...
FED_27_28=b'''<html><body><h2>2027 FOMC Meetings</h2>January 26-27 March 16-17 April 27-28 June 8-9 July 27-28 September 14-15 October 26-27 December 7-8 <h2>2028 FOMC Meetings</h2>January 25-26 March 14-15 May 2-3 June 13-14 July 25-26 September 19-20 October 31-November 1 December 12-13 Note: Future dates are tentative.</body></html>'''


def fetcher(bls_ics=BLS,cpi=BLS_CPI_HTML,jobs=BLS_JOBS_HTML,bea_raw=BEA,bea_next_raw=BEA_EMPTY,fed=FED):
 def f(url):
  if url==c.BLS_ICS:
  ...
  if url==c.BEA_SCHEDULE:return bea_raw
...
class CalendarProbeTests(unittest.TestCase):
 ...
 def test_bea_complete_year_rejects_wrong_reference_period_set(self):
  e=c.parse_bea(BEA_NEXT,2027,c.BEA_NEXT_YEAR);e[-1]['period']='2027-12';self.assertFalse(c.complete_monthly_year(e,2027))
 ...
 def test_jst_jan1_probe_rolls_2027_to_2028_without_year_constants(self):
  err=urllib.error.HTTPError(c.BLS_ICS,403,'Forbidden',None,None)
  r=c.probe(fetcher(bls_ics=err,cpi=BLS_CPI_27_28,jobs=BLS_JOBS_27_28,bea_raw=BEA_2027,bea_next_raw=BEA_2028,fed=FED_27_28),NOW_2027_JST)
  self.assertEqual(c.local_year(NOW_2027_JST),2027)
  self.assertEqual(r['source_modes']['bls'],'html_fallback_current_plus_next_year')
  self.assertEqual(r['source_modes']['bea'],'primary_current_plus_next_year')
  self.assertEqual(r['source_modes']['fed'],'primary_current_plus_next_year')
  self.assertEqual(len(r['candidate_events']),88)
  for family,count in {'cpi':12,'jobs':12,'pce':12,'fomc':8}.items():
   self.assertEqual(len([e for e in r['candidate_events'] if e['family']==family and e['date'].startswith('2027-')]),count)
   self.assertEqual(len([e for e in r['candidate_events'] if e['family']==family and e['date'].startswith('2028-')]),count)
